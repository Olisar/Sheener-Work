<?php
/* File: sheener/php/php/syncdown.php */

/**
 * Sync down script - Copies ONLY NEW or UPDATED files from D:\sheener
 * to local C:\xampp0\htdocs\sheener
...
// Configuration
$source = 'D:/sheener';          // Backup source (D:)
$destination = dirname(__DIR__); // Local project: C:\xampp0\htdocs\sheener
...
function syncDownFiles($src, $dst) {
    if (!is_dir($src)) {
    ...
    foreach ($iterator as $item) {
        ...
        if ($item->isDir()) {
            ...
            // Copy if destination doesn't exist OR source (D:) is newer
...
// Execute sync down
try {
    $filesCopied = syncDownFiles($source, $destination);
